<?php
    ...
    // Show Data per Page
    $dataPerPage = 5;
    // Summary of all row in videogames
    $dataSummary = count(query("SELECT * FROM videogames"));
    // Summary of all pages
    $pageSummary = ceil( $dataSummary / $dataPerPage );
    // Active Page
    $pageActive = (isset($_GET["page"])) ? $_GET["page"] : '1';
    // First Data in Every Page Algorithm
    $dataFirst = ($pageActive * $dataPerPage) - $dataPerPage;
    
    // PAGINATION ALGORITHMS
    $paginationCol = ($paginationCol > $pageSummary) ? $pageSummary: $paginationCol;
    ...
    if (isset($_POST["search"])){
        ...
        if( isset($videogames) ){
            ...
            $pageActive = ceil($rowNum / $dataPerPage);
            $dataFirst = 0;
        }
    ...
    <div class="container" style="margin-top: 100px;">
        ...
        <div class="row">
            ...
            <div class="col-md-8 mb-2">
                <nav aria-label="Page navigation example success">
                    <ul class="pagination justify-content-center mb-0">

                        <?php if ($pageActive > 1) : ?>
                            <li class="page-item">
                                <a class="page-link fs-5" href="index.php?page=<?= $pageActive - 1 ?>" aria-label="Previous">
                        ...
                        <?php
                            ...
                            if ($pageActive > ($pageSummary - (floor($paginationCol / 2)))){
                                $min = $pageSummary - ($paginationCol - 1);
                                $max = $pageSummary;
                            } else if ( $pageActive > (floor($paginationCol / 2)) ){
                                $min = $pageActive - (floor($paginationCol / 2));
                                $max = $pageActive + (floor($paginationCol / 2));
                            } else {
                        ...
                        <?php if ($pageActive < $pageSummary) : ?>
                        <li class="page-item">
                            <a class="page-link fs-5" href="index.php?page=<?= $pageActive + 1 ?>" aria-label="Next">
